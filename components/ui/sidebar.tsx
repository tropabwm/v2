...
import {
  ...
  Megaphone // <<< ADICIONADO Megaphone
} from "lucide-react";
...
const navItems: NavItem[] = [ 
    ...
    { href: "/campaign-manager", icon: Megaphone, label: "Gestão de Tráfego" }, // <<< NOVO ITEM ADICIONADO
...
const Sidebar: React.FC<SidebarProps> = ({ isHidden }) => {
  ...
  const textNeonStyle = { textShadow: `0 0 4px ${NEON_COLOR}` }; 
           ...
           {navItems.map((item) => {
              ...
              const linkContent = ( <>
                  <item.icon className={cn(
                      ...
                      isActive ? `text-white` : `text-[${NEON_COLOR}]` 
              ...
              return (
                  <Link key={item.href} href={item.href} className={navItemBaseStyle} aria-current={isActive ? "page" : undefined} > {/* Revertido para item.href como key */}
                      {linkContent}
                  </Link>
              );
            })}

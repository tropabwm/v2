...
export default function CreativesPage() {
  ...
  const [playerKey, setPlayerKey] = useState<number>(0);
   ...
   const getStatusBadgeClass = (status?: string) => cn("absolute top-2 left-2 text-[9px] border px-1.5 py-0 h-4 inline-flex items-center rounded-full shadow-sm leading-none z-10", statusColors[status || 'draft'] || statusColors['draft']);
   ...
   const handleSave = async (e: React.FormEvent) => {
     ...
     if (!isMedia && !formData.content?.trim()) {
     ...
     try {
       ...
       toast({ title: "Erro ao Salvar", description: error.response?.data?.message || error.message || "Falha desconhecida.", variant: "destructive" });
     } finally {
       setIsSaving(false);
     }
   };
   ...
   const getVideoUrl = (content: string | undefined | null): string => {
    ...
    const getCreativePreview = (creative: Creative) => {
        ...
        if (!contentString && ['headline', 'body', 'cta'].includes(creative.type)) {
            return <div className={cn(basePreviewContainerStyle, textStyle)} title={`Conteúdo vazio (${creative.type})`}>...</div>;
        }
        ...
        try {
            ...
            if (creative.type === 'video') {
                if (thumbnailUrl) { // Prioriza a thumbnail da API de upload
                     ...
                     return (
                        <div className={basePreviewContainerStyle}>
                            <img key={`${creative.id}-thumb`} src={thumbSrc} alt={`${creative.name} (thumbnail)`} className={mediaStyle} loading="lazy"
                                onError={(e) => {
                  ...
                  >
                      ...
                      <div className={cn(
                          ...
                          {creative.campaign_id && ( 
                           ...
                           {creative.comments && viewMode === 'grid' && (
                                <Tooltip>
                             ...
                             {viewMode === 'list' && creative.comments && (
                                 <Tooltip>
                                    <TooltipTrigger asChild>
              ...
              {['image', 'video'].includes(formData.type || '') ? (
                <>
                  ...
                  <div className="space-y-2">
                    {(formData.content && !fileToUpload && (formData.content.startsWith('http') || formData.content.startsWith('/uploads/'))) ? (
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <Button type="button" variant="ghost" size="sm" className="h-7 w-7 p-0" onClick={() => setFormData(prev => ({...prev, content: '', thumbnailUrl: null}))}>
                ...
                <div className="text-center py-4 text-sm text-gray-400">
                  Selecione um tipo de criativo para ver as opções de conteúdo.
                </div>
              )}
            </div>
      ...
      <Dialog open={isDetailOpen} onOpenChange={setIsDetailOpen}>
        <DialogContent className={cn(cardStyle, "max-w-2xl md:max-w-4xl max-h-[90vh]")}>
          ...
          <ScrollArea className="max-h-[calc(90vh-200px)]">
            <div className="space-y-4 p-1 pr-3">
                ...
                {selectedCreativeDetail?.type === 'image' && selectedCreativeDetail.content && (
                  <div className="mt-2 text-center">
                    ...
                    <div className="mt-2 flex justify-center gap-2">
                      <Button size="sm" variant="outline" className={neumorphicButtonStyle} onClick={() => window.open(selectedCreativeDetail.content.startsWith('/uploads/') ? `/api/file${selectedCreativeDetail.content}` : selectedCreativeDetail.content, '_blank')}><ExternalLink className="h-3.5 w-3.5 mr-1" /> Abrir Original </Button>
                      <Button size="sm" variant="outline" className={neumorphicButtonStyle} onClick={() => { closeCreativeDetail(); openForm(selectedCreativeDetail); }}><Edit className="h-3.5 w-3.5 mr-1" /> Editar </Button>
                    </div>
                  </div>
                )}
                  ...
                  <div className="mt-2 text-center">
                    <div className="rounded overflow-hidden max-w-full bg-black/50 mx-auto" style={{ maxWidth: '560px' }}>
                      <div className="aspect-video">
                        <ReactPlayer url={getVideoUrl(selectedCreativeDetail.content)} controls width="100%" height="100%" key={playerKey} onError={(e) => console.error("Video error:", e)} />
                      </div>
                    </div>
                    <div className="mt-2 flex justify-center gap-2">
                       <Button size="sm" variant="outline" className={neumorphicButtonStyle} onClick={() => window.open(getVideoUrl(selectedCreativeDetail.content), '_blank')}><ExternalLink className="h-3.5 w-3.5 mr-1" /> Abrir Original </Button>
                       <Button size="sm" variant="outline" className={neumorphicButtonStyle} onClick={() => { closeCreativeDetail(); openForm(selectedCreativeDetail); }}><Edit className="h-3.5 w-3.5 mr-1" /> Editar </Button>
                    </div>
                  </div>
                )}
                ...
                {['headline', 'body', 'cta'].includes(selectedCreativeDetail?.type || '') && selectedCreativeDetail?.content && (
                  <div className="mt-2">
                    ...
                    <div className="mt-2 flex justify-end gap-2">
                      <Button size="sm" variant="outline" className={neumorphicButtonStyle} onClick={() => { navigator.clipboard.writeText(selectedCreativeDetail.content); toast({ title: "Copiado", description: "Texto copiado para área de transferência" }); }}><CopyIcon className="h-3.5 w-3.5 mr-1" /> Copiar </Button>
                      <Button size="sm" variant="outline" className={neumorphicButtonStyle} onClick={() => { closeCreativeDetail(); openForm(selectedCreativeDetail); }}><Edit className="h-3.5 w-3.5 mr-1" /> Editar </Button>
                    </div>
                  </div>
                )}
              ...
              <div>
                ...
                <div className="space-y-2">
                  <Textarea value={detailComment} onChange={(e) => setDetailComment(e.target.value)} placeholder="Adicione comentários, instruções ou notas sobre este criativo..." className={neumorphicTextAreaStyle} />
                  <div className="flex justify-end">
                    <Button size="sm" variant="outline" className={cn(primaryButtonStyle, "h-7 px-3 text-xs")} onClick={handleSaveComment} disabled={isSavingComment || detailComment === (selectedCreativeDetail?.comments || '')} >
                      {isSavingComment ? <Loader2 className="h-3 w-3 animate-spin mr-1" /> : <Save className="h-3 w-3 mr-1" />} Salvar Comentário
                    </Button>
                  </div>
                </div>
              </div>
          ...
          <DialogFooter className="mt-4">
            <DialogClose asChild>
              <Button className={neumorphicButtonStyle}> Fechar </Button>
            </DialogClose>
          </DialogFooter>
        </DialogContent>
      </Dialog>

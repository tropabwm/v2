...
interface DailyMetric { date: string; revenue?: number; clicks?: number; conversions?: number; cost?: number; }
interface MetricsTotals { revenue?: number; clicks?: number; conversions?: number; cost?: number; }
interface MetricsApiResponse { totals: MetricsTotals; dailyData: DailyMetric[]; }
interface ComparisonPoint { day: number; period1Value?: number; period2Value?: number; }
interface TooltipState { x: number; y: number; visible: boolean; value: number; day: number; periodLabel: string; periodColor: string; }
interface CampaignOption { id: string; name: string; } // ID da campanha geralmente é string (UUID ou numérico como string)
...
export default function DatesPage() {
    ...
    const fetchCampaigns = useCallback(async () => {
        ...
        try {
            ...
            // Certificar que IDs sejam strings para o Select
            const campaignData = response.data?.map(c => ({ ...c, id: String(c.id) })) || [];
    ...
    const fetchPeriodData = async (startDate: string, endDate: string, campaignId: string | undefined): Promise<MetricsApiResponse | null> => {
        try {
            const response = await axios.get<MetricsApiResponse>('/api/metrics', {
                params: { startDate, endDate, campaignId: campaignId === 'all' ? undefined : campaignId }
            });
    ...
    const fetchAndComparePeriods = useCallback(async () => {
        ...
        setComparisonData([]); setTotalDifference(null); setPercentDifference(null); setPeriod1Total(null); setPeriod2Total(null);

        const campId = selectedCampaignId === 'all' ? undefined : selectedCampaignId;
        const metricToCompare: keyof MetricsTotals & keyof DailyMetric = 'revenue';
        ...
        try {
            ...
            const percentChange = total1 !== 0 ? (difference / Math.abs(total1)) * 100 : (total2 !== 0 ? Infinity : 0);


            setTotalDifference(difference);
            setPercentDifference(isFinite(percentChange) ? percentChange : (percentChange === Infinity ? Infinity : null) );
            ...
            for (let i = 0; i < maxLength; i++) {
                formattedComparisonData.push({
                    day: i + 1,
                    period1Value: dailyData1[i]?.[metricToCompare] ?? undefined,
    ...
    const renderLineChart = () => {
         ...
         const allValues = comparisonData.flatMap(d => [d.period1Value, d.period2Value]).filter(v => typeof v === 'number') as number[];
         const maxValue = allValues.length > 0 ? Math.max(...allValues, 0) : 1; // Max de todos os valores ou 0, com fallback 1
         ...
         const yScale = (value: number) => height - padding - Math.max(0, (value / (maxValue === 0 ? 1 : maxValue))) * (height - 2 * padding); // Evita divisão por zero se maxValue for 0

         const getBezierPath = (points: {x: number, y: number}[]) => {
             if (points.length < 1) return ''; if (points.length === 1) return `M ${points[0].x.toFixed(2)},${points[0].y.toFixed(2)}`;
         ...
         return ( <div className="relative"> <svg width="100%" height={height} viewBox={`0 0 ${width} ${height}`}> <defs> <filter id="glow" x="-50%" y="-50%" width="200%" height="200%"> <feGaussianBlur stdDeviation="2" result="coloredBlur"/> <feMerge> <feMergeNode in="coloredBlur"/> <feMergeNode in="SourceGraphic"/> </feMerge> </filter> </defs> <g> {gridLines} <line x1={padding} y1={height - padding} x2={width - padding} y2={height - padding} stroke="#a0a0a0" strokeWidth="0.5"/> <line x1={padding} y1={padding} x2={padding} y2={height - padding} stroke="#a0a0a0" strokeWidth="0.5"/> <text x={width / 2} y={height - 8} fill="#a0a0a0" fontSize="11" textAnchor="middle" > Dia no Período </text> <text x={padding - 35} y={height/2} fill="#a0a0a0" fontSize="11" textAnchor="middle" transform={`rotate(-90 ${padding-35},${height/2})`} > Receita (R$) </text> </g> {period1Path && <path d={period1Path} fill="none" stroke={neonColor} strokeWidth="2" style={{ filter: 'url(#glow)' }} />} {period2Path && <path d={period2Path} fill="none" stroke={neonGreenColor} strokeWidth="2" style={{ filter: 'url(#glow)' }} />} {period1Points.map((point, index) => ( <circle key={`p1-${index}`} cx={point.x} cy={point.y} r="3.5" fill={neonColor} stroke="#0e1015" strokeWidth="1" className="cursor-pointer transition-all duration-150 ease-out hover:r-5" onMouseOver={() => handleMouseOver(point, 'Período 1', neonColor)} onMouseOut={handleMouseOut} /> ))} {period2Points.map((point, index) => ( <circle key={`p2-${index}`} cx={point.x} cy={point.y} r="3.5" fill={neonGreenColor} stroke="#0e1015" strokeWidth="1" className="cursor-pointer transition-all duration-150 ease-out hover:r-5" onMouseOver={() => handleMouseOver(point, 'Período 2', neonGreenColor)} onMouseOut={handleMouseOut} /> ))} </svg> <div className={cn( "absolute z-10 pointer-events-none", "bg-[#13151a]/90 backdrop-blur-sm", "border border-[#2d62a3]/50 rounded-md shadow-lg", "px-3 py-1.5 text-xs text-white whitespace-nowrap", "transition-opacity duration-200 ease-out", tooltip.visible ? "opacity-100" : "opacity-0", "transform -translate-x-1/2 -translate-y-[calc(100%+8px)]" )} style={{ left: tooltip.x, top: tooltip.y }} > <span className="font-semibold" style={{ color: tooltip.periodColor }}> {tooltip.periodLabel} </span> - Dia {tooltip.day}: <br/> <span className="font-bold">{formatCurrency(tooltip.value)}</span> </div> </div> );
    };
                        ...
                        <CardContent className="p-0 space-y-4 overflow-y-auto custom-scrollbar flex-grow">
                                    ...
                                    <SelectContent className="bg-[#1e2128] border-[#1E90FF]/30 text-white">
                                        {isLoadingCampaigns ? ( <SelectItem value="loading" disabled>Carregando...</SelectItem> ) : (
                                            <>
                                                ...
                                                {campaigns.length > 0 ? campaigns.filter(c => c && c.id).map((c) => ( // Adicionado filtro para c.id
                                                    <SelectItem key={c.id} value={c.id} className="text-xs hover:bg-[#1E90FF]/20 focus:!bg-[#1E90FF]/20">{c.name}</SelectItem>
                                                )) : (
                            ...
                            <div className="space-y-1">
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                    ...
                                    <div> <Label htmlFor="endDate1" className="text-xs text-gray-400 mb-1 block">Data Final</Label> <Input id="endDate1" type="date" value={period1.endDate} onChange={(e) => handleInputChange(1, 'endDate', e.target.value)} className={cn(neumorphicInputStyle, "h-8 px-2 text-xs")} disabled={isLoadingComparison} min={period1.startDate || undefined} /> </div>
                                </div>
                            </div>
                            <div className="space-y-1">
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                    ...
                                    <div> <Label htmlFor="endDate2" className="text-xs text-gray-400 mb-1 block">Data Final</Label> <Input id="endDate2" type="date" value={period2.endDate} onChange={(e) => handleInputChange(2, 'endDate', e.target.value)} className={cn(neumorphicInputStyle, "h-8 px-2 text-xs")} disabled={isLoadingComparison} min={period2.startDate || undefined} /> </div>
                                </div>
                            </div>
                        </CardContent>
                    ...
                    <div className="lg:col-span-2 space-y-4 overflow-y-auto custom-scrollbar">
                        {isLoadingComparison ? (
                             <Card className={cn(cardStyle, "flex items-center justify-center h-[400px] p-6")}>
                                 <Loader2 className="h-10 w-10 animate-spin text-primary" style={{color: neonColor}}/>
                             </Card>
                        ) : apiError ? (
                            ...
                            <>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <Card className={cn(cardStyle, "p-4 text-center")}>
                                        ...
                                        <CardContent className="p-0"> <p className="text-xl font-bold text-white" style={{ color: neonColor }}>{formatCurrency(period1Total ?? undefined)}</p> </CardContent>
                                    </Card>
                                    <Card className={cn(cardStyle, "p-4 text-center")}>
                                        ...
                                        <CardContent className="p-0"> <p className="text-xl font-bold text-white" style={{ color: neonGreenColor }}>{formatCurrency(period2Total ?? undefined)}</p> </CardContent>
                                    </Card>
                                    <Card className={cn(cardStyle, "p-4 text-center")}>
                                        ...
                                        <CardContent className="p-0"> <p className={`text-xl font-bold ${(totalDifference ?? 0) >= 0 ? "text-green-400" : "text-red-400"}`} style={{ textShadow: `0 0 5px ${(totalDifference ?? 0) >= 0 ? neonGreenColor : neonRedColor}` }}> {formatCurrency(totalDifference ?? undefined)} </p> </CardContent>
                                    </Card>
                                    <Card className={cn(cardStyle, "p-4 text-center")}>
                                        ...
                                        <CardContent className="p-0"> <p className={`text-xl font-bold ${(percentDifference ?? 0) >= 0 ? "text-green-400" : "text-red-400"}`} style={{ textShadow: `0 0 5px ${(percentDifference ?? 0) >= 0 ? neonGreenColor : neonRedColor}` }}> {(percentDifference === Infinity || (percentDifference && percentDifference > 0)) ? <TrendingUp className="inline h-5 w-5 mr-1"/> : (percentDifference !== null && percentDifference < 0 ? <TrendingDown className="inline h-5 w-5 mr-1"/> : '')} {percentDifference === Infinity ? "∞" : formatPercent(percentDifference)} </p> </CardContent>
                                    </Card>
                                </div>
                             ...
                             <Card className={cn(cardStyle, "flex items-center justify-center h-[400px] p-6")}> <div className="text-center"> <CalendarDays size={48} className="text-primary opacity-30 mx-auto mb-4" style={{ filter: `drop-shadow(0 0 8px ${neonColor})`, color: neonColor }}/> <p className="text-lg text-gray-300" style={{ textShadow: `0 0 4px ${neonColorMuted}` }}> Selecione campanha e períodos </p> <p className="text-lg font-semibold text-primary mb-1" style={{ textShadow: `0 0 6px ${neonColor}`, color: neonColor }}>e clique em "Comparar"</p> <p className="text-sm text-gray-500">para visualizar o gráfico e as diferenças.</p> </div> </Card>
                        )}
                    </div>
                </div>
            </div>

...
import { CalendarIcon, DollarSign, Info, Megaphone, Users, Briefcase, Link2, Percent, Settings } from 'lucide-react'; // Adicionado Link2, Percent, Settings
...
export interface ClientAccountOption {
  id: string;
  name: string;
  platform: 'google' | 'meta' | 'tiktok' | 'manual' | string; // Adicionado tiktok, manual
...
export interface CampaignFormData {
  ...
  external_campaign_id?: string | null; // Novo campo
  // Adicione outros campos se necessário (ex: cost_traffic, etc.)
}
...
export default function CampaignManagerForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.selectedClientAccountId) {
        // Substituir alert por toast se preferir
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className={cn(cardStyle, "max-w-3xl md:max-w-4xl p-0 border-none")}>
        ...
        <form onSubmit={handleSubmit}>
          <ScrollArea className="max-h-[calc(85vh-120px)]"> {/* Altura da área de rolagem */}
            <div className="p-4 space-y-1"> {/* Espaçamento entre acordeões */}
              <Accordion type="multiple" defaultValue={["infoPrincipais", "plataformaObjetivosFormatos", "orcamentoDatas"]} className="w-full">
                ...
                <AccordionItem value="metricasNegocio" className="border-none"> {/* Último item, sem mb-2 */}
                  ...
                  <AccordionContent className="pt-2 px-0.5 pb-0.5">
                     <div className="p-4 grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-3 bg-[#0A0B0F]/30 rounded-md border border-transparent hover:border-[#1E90FF]/20 transition-colors">
                        ...
                        {/* Adicionar mais campos como Custo por Lead (CPL) esperado, etc. */}
                    </div>
                  </AccordionContent>
                </AccordionItem>
